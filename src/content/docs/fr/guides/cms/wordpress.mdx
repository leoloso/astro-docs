...
WordPress est doté d'un système intégré de l'[API REST WordPress](https://developer.wordpress.org/rest-api/) pour connecter vos données WordPress à Astro. Vous pouvez éventuellement installer [WPGraphQL](https://wordpress.org/plugins/wp-graphql/) ou [Gato GraphQL](https://wordpress.org/plugins/gatographql/) sur votre site afin d'utiliser GraphQL.
